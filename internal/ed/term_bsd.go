...
// SaveTermios saves the current termios for fd on BSD-like systems.
func SaveTermios(fd int) (unix.Termios, error) {
	// On BSD the wrapper returns a pointer; reuse it safely.
	t := unix.Termios{}
...
// RestoreTermios restores a previously saved termios for fd on BSD-like systems.
func RestoreTermios(fd int, t unix.Termios) error {
	return unix.IoctlSetTermios(fd, unix.TIOCSETA, &t)
}

// SetRaw sets terminal to a raw-ish mode and returns previous termios on BSD-like systems.
