...
func (e *Engine) run(code, filename string, args []string) error {
	...
	// Register builtins from package-level factories. This moves the large
	// collection of helper registrations out of the main engine for clarity.
	for name, fn := range Builtins(e) {
		_ = script.Add(name, fn)
	}
