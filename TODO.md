...
### Tengo runtime
- [ ] Add `github.com/d5/tengo/v3` dependency
- [ ] `run <script.tengo>` builtin — execute a Tengo script with args
- [ ] `eval '<expr>'` builtin — inline one-liner execution
- [ ] Blessed script path: `$KUSH_SCRIPTS` (default `~/.kush/scripts/`)
- [ ] Auto-register `.tengo` files in blessed path as shell builtins
- [ ] Preloaded `kush` module: env get/set, args, cwd, exit, prompt invalidation
...
- [ ] `http` module: expose the native HTTP functions to scripts

### Native HTTP builtins
- [ ] `get <url>` — HTTP GET, stdout output, pipe-friendly, `-H` headers, `-j` JSON parse
- [ ] `post <url> <data>` — HTTP POST, auto content-type detection
- [ ] `put <url> <data>` — HTTP PUT
- [ ] `delete <url>` — HTTP DELETE
- [ ] `head <url>` — HTTP HEAD, show response headers
- [ ] `fetch <url> [-o file]` — download to file (wget/curl equivalent)
- [ ] Internal HTTP client shared between builtins and Tengo `http` module
